...
def myamr_to_seq(amr,snt_token,lemma_token,pos,rl,fragment_to_node_converter,high_freq):   #high_freq should be a dict()
    ...
    old_unaligned_index = [i for i in range(l) if not ( pos[i]  in ["IN","POS"] or  lemma_token[i]=="would" or  lemma_token[i]=="will" and pos[i]=="MD"
  ...
  #  assert n_snt <= n_amr or unaligned_index != [],(n_amr,n_snt,concepts,snt_token,amr
    ...
    for amr_index,role_list in rel:
        amr_concept = uni_to_list(amr_index[0]) #if align else  uni_to_list(AMRUniversal(UNK_WORD,output_concepts[amr_index[1]][AMR_CAT],NULL_WORD))
...
def filter_non_aligned(input_concepts,rel,unaligned_index):
    ...
    for i,data in enumerate(input_concepts):
        if len(data[-1]) == 0:
            output_concepts.append([NULL_WORD,NULL_WORD,NULL_WORD,NULL_WORD,0,unaligned_index])   #len(amr) >= len(snt)
...
def handle_sentence(data,filepath,build_dict,n,word_only):
    ...
    if build_dict:
        ...
        data["amr_rel_id"],data["amr_rel_index"],data["roles_mat"],data["root"] = rel_seq_to_id(lemma_dict,category_dict,sensed_dict,rel_dict,rel)
...
fragment_to_node_converter = ReCategorizor(from_file=True,path="data/graph_to_node_dict_extended"+with_jamr,training=False,auto_convert_threshold=opt.threshold)
...
print ("initial converted,threshold,len(non_rule_set),high_text_num,high_frequency,low_frequency,low_text_num,high_freq")
high_freq ={**high_text_num,**high_frequency}

#high_freq =high_frequency

print ("initial converted",threshold,len(non_rule_set),len(high_text_num),len(high_frequency),len(low_frequency),len(low_text_num),len(high_freq))
...
fragment_to_node_converter = ReCategorizor(from_file=True,path="data/graph_to_node_dict_extended"+with_jamr,training=False,ner_cat_dict=aux_dict)
...
print(len(word_dict),len( lemma_dict),len( pos_dict),len( ner_dict ),len(high_dict),len(sensed_dict),len(category_dict),len(aux_dict),len(rel_dict))
