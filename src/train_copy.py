...
parser.add_argument('-total_size', type=int, default=512,
...
def my_loss(out,high_index,rule_index,tgtBatch,n_freq,n_cat,eval):
 ...
 #   print ("tgtBatch",tgtBatch.size())
    lemma_batch = tgtBatch[0].view(-1)
    cat_batch = tgtBatch[1].view(-1)

    high_index = high_index.view(-1)
    rule_index = rule_index.view(-1,src_len)

    high = out[:,:,:,0:n_freq].contiguous().view(-1,src_len,n_freq)
    rule = out[:,:,:,n_freq].contiguous().view(-1,src_len)
    cat = out[:,:,:,n_freq+1:].contiguous().view(-1,src_len,n_cat)
    
    high_prob = Variable(high.data, requires_grad= not eval , volatile= eval)
    rule_prob = Variable(rule.data, requires_grad=  not eval , volatile= eval)
    cat_prob = Variable(cat.data, requires_grad=  not eval , volatile= eval)
...
def memoryEfficientLoss(outputs,attns,tgtBatch, srcBatch, high_index,lemma_index,generator,n_freq,n_cat,dicts,eval):
    ''' high_index: tgt_len x batch_size        (0 ... n_high-1)
        rule_index: tgt_len x batch_size x src_len  (0 1)
        cat_batch:  tgt_len x batch_size
        lemma_batch:  tgt_len x batch_size
    ...
    outputs,attns,tgtBatch, srcBatch, high_index,lemma_index


    outputs_split = torch.split(outputs, opt.max_generator_batches)
    attns_split =  torch.split(attns, opt.max_generator_batches)
    tgt_split = torch.split(tgtBatch, opt.max_generator_batches)
    high_index_split = torch.split(high_index, opt.max_generator_batches)
    lemma_index_split = torch.split(lemma_index,opt.max_generator_batches)

    out_grads = []
    posteriors = []
    for out_t, tgt_t ,attn_t,high_index_t,lemma_index_t in zip(outputs_split, tgt_split,attns_split,high_index_split,lemma_index_split):

        out = generator(attn_t,out_t,tgt_t,srcBatch[LEMMA])

        loss_t,num_words_t,posterior,out_grad = my_loss(out,high_index_t,lemma_index_t,tgt_t,len(dicts["concept_ls"]),len(dicts["category_dict"]),eval)
    ...
    return loss,num_words,posteriors,out_grads
...
def eval(model, data,dicts):
   ...
   #     print ("in eval tgtBatch",tgtBatch.size())
        loss,num_words,posterior = my_loss(out,high_index,lemma_index,tgtBatch,len(dicts["concept_ls"]),len(dicts["category_dict"]),True)
...
def trainModel(model, trainData, validData, dicts, optim):
   ...
   #     print (len(trainData))
     ...
     #       print ("trainModel",srcBatch.size())
       ...
       #     loss,num_words, grad_output = memoryEfficientLoss(out,attns,tgtBatch, srcBatch, generator, high_index,lemma_index,len(dicts["concept_ls"]),len(dicts["category_dict"]))
            out = model.forward(x)
     ...
     #       del out,out_grad,x
        ...
        #        print ("trainModel",srcBatch.size())
                ...
                print("perplexity", torch.exp(1.0*report_loss/report_words).data[0])
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        print('Train perplexity: %g' % math.exp(train_loss.data[0]))
